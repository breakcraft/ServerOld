        ...
        if (!Environment.NODE_PRODUCTION && player.staffModLevel >= 4) {
            if (cmd[0] === Environment.NODE_DEBUGPROC_CHAR) {
                ...
                for (let i = 0; i < script.info.parameterTypes.length; i++) {
                    ...
                    try {
                        switch (type) {
                            case ScriptVarType.STRING: params[i] = args.shift() ?? ''; break;
                            case ScriptVarType.INT: params[i] = tryParseInt(args.shift() ?? '0', 0); break;
                            case ScriptVarType.OBJ:
                            case ScriptVarType.NAMEDOBJ: params[i] = ObjType.getId(args.shift() ?? ''); break;
                            case ScriptVarType.NPC: params[i] = NpcType.getId(args.shift() ?? ''); break;
                            case ScriptVarType.LOC: params[i] = LocType.getId(args.shift() ?? ''); break;
                            case ScriptVarType.SEQ: params[i] = SeqType.getId(args.shift() ?? ''); break;
                            case ScriptVarType.STAT: params[i] = PlayerStatMap.get((args.shift() ?? '').toUpperCase()) ?? -1; break;
                            case ScriptVarType.INV: params[i] = InvType.getId(args.shift() ?? ''); break;
                            ...
                            case ScriptVarType.INTERFACE: params[i] = Component.getId(args.shift() ?? ''); break;
                            case ScriptVarType.SPOTANIM: params[i] = SpotanimType.getId(args.shift() ?? ''); break;
                            case ScriptVarType.IDKIT: params[i] = IdkType.getId(args.shift() ?? ''); break;
                            default: break;
                        }
                    } catch {
                        // invalid arguments
        ...
        if (player.staffModLevel >= 3) {
            switch (cmd) {
                case 'setvar': {
                    ...
                    break; // ← break fixes ESLint no‑fallthrough
                }
                ...
                case 'normalrun': {
                    ...
                    if (ep._originalUpdateEnergy) {
                        ...
                        delete ep._originalUpdateEnergy;
                        player.messageGame('Unlimited run energy disabled.');
                    } else {
                        player.messageGame('Unlimited run not active.');
                    }
                    break;
                }
                ...
                case 'item': {
                    ...
                    /* ── 2. Strip optional amount (only if ≥2 tokens remain) ── */
                    let amount = 1;
                    if (args.length >= 2) {
                    ...
                    if (args.length === 1 && !isNaN(tryParseInt(args[0], NaN))) {
                        ...
                        if (itemId === -1) {
                            ...
                            for (let i = 0; i < ObjType.count; i++) {
                                const o = ObjType.get(i);
                                if (o?.name?.toLowerCase().includes(term)) hits.push({ id: i, name: o.name });
                            }
                ...
                case 'id': {
                    ...
                    if (exactId !== -1) {
                        ...
                        if (!hits.length) {
                            ...
                            /* -------------- NEW: send each row individually -------------- */
                            player.messageGame(`Multiple items${hits.length > 10 ? ' (showing first 10)' : ''}:`);
                            hits.slice(0, 10).forEach(h => player.messageGame(`- ${h.name} (ID: ${h.id})`));
                            if (hits.length > 10) {
                                player.messageGame(`... and ${hits.length - 10} more.`);
                ...
                case 'givecrap': {
                    for (let i = 0; i < 28; i++) {
                        ...
                        while (random === -1) {
                            ...
                            if ((!Environment.NODE_MEMBERS && obj.members) || obj.dummyitem) random = -1;
                        }
                ...
                case 'locadd': {
                    ...
                    World.addLoc(
                        new Loc(
                            player.level, player.x, player.z, lt.width, lt.length,
                            EntityLifeCycle.DESPAWN, lt.id,
                            LocShape.CENTREPIECE_STRAIGHT, LocAngle.WEST
                        ),
                        500
                    );
                ...
                case 'npcadd': {
                    ...
                    World.addNpc(
                        new Npc(
                            player.level, player.x, player.z, nt.size, nt.size,
                            EntityLifeCycle.DESPAWN, World.getNextNid(),
                            nt.id, nt.moverestrict, nt.blockwalk
                        ),
                        500
                    );
                    break;
        ...
        if (player.staffModLevel >= 2) {
            switch (cmd) {
                ...
                case 'tele': {
                    ...
                    if (
                        level < 0 || level > 3 || mx < 0 || mx > 255 ||
                        mz < 0 || mz > 255 || lx < 0 || lx > 63 || lz < 0 || lz > 63
                    ) {
                        return false;
                    }
